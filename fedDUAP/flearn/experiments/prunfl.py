import numpy as np
from FL_pruning_algorithm.fedDUAP.flearn.utils.model_utils import average_weights, test_inference, is_conv
...
from FL_pruning_algorithm.fedDUAP.flearn.utils.prune import get_ratio, lt_prune
from FL_pruning_algorithm.fedDUAP.data.util import get_client_server_noniid_degree, get_global_distribution, get_target_users_distribution, get_noniid_degree
import copy
import os
import time
from FL_pruning_algorithm.fedDUAP.flearn.experiments.central import CentralTraining


class PrunFL(CentralTraining):
    """
    通过计算剪枝率，对模型，从小到达进行剪枝
    """
    def __init__(self, args, share_percent=0, iid=True, unequal=False, result_dir="prunfl",
        ...
        for epoch in range(self.args.epochs):
            ...
            if epoch == self.prune_interval:
                if self.args.dataset == "cifar10":
                    from FL_pruning_algorithm.fedDUAP.data.cifar10.cifar10_data_pt import get_pt_dataset
                    train_dataset_pt = get_pt_dataset()
                elif self.args.dataset == "cifar100":
                    from FL_pruning_algorithm.fedDUAP.data.cifar100.cifar100_data_pt import get_pt_dataset
                    train_dataset_pt = get_pt_dataset()
