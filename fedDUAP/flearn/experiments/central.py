...
import numpy as np
import random
from FL_pruning_algorithm.fedDUAP.flearn.utils.update import LocalUpdate
from FL_pruning_algorithm.fedDUAP.flearn.utils.model_utils import average_weights, test_inference, is_conv, ratio_combine, ratio_minus
from FL_pruning_algorithm.fedDUAP.flearn.utils.options import args_parser
from FL_pruning_algorithm.fedDUAP.flearn.utils.util import record_log, save_result
from FL_pruning_algorithm.fedDUAP.flearn.models import cnn, vgg, resnet, lenet
from FL_pruning_algorithm.fedDUAP.data.util import get_client_server_noniid_degree, get_global_distribution, get_target_users_distribution, \
    get_noniid_degree
import copy
import os
import time
...
class CentralTraining(object):
    ...
    def __init__(self, args, share_percent=0, iid=True, unequal=False,
        ...
        self.l = 2  # noniid的程度， l越小 noniid 程度越大， 当 l = 1 时， 就是将数据按照顺序分成 clients 份，每个设备得到一份， 基本上只包含一个数字
    ...
    def init_data(self):
        if self.args.dataset == "cifar10":
            from FL_pruning_algorithm.fedDUAP.data.cifar10.cifar10_data import get_dataset
            self.num_data = 40000
            ...
            from FL_pruning_algorithm.fedDUAP.data.cifar100.cifar100_data import get_dataset
            self.num_data = 40000
            self.num_share = int(self.num_data * self.share_percent / 100)
            self.l = self.l * 10
    ...
    def record_base_message(self, log_path):
        ...
        record_log(self.args, log_path,
                   f"=== noniid_degree client/share/share_l: {client_noniid_degree} / {share_noniid_degree} /{self.args.share_l} ===\n")
        record_log(self.args, log_path,
        ...
        for idx in idxs_users:
            ...
            if self.args.optim == "fsgdm":
                P, v, loss = local_model.update_weights(
                    model=copy.deepcopy(global_model), global_round=epoch, mu=self.args.client_mu, v=copy.deepcopy(self.v))
                if loss < train_losses[0] * 3:
                    local_P.append([len(user_groups[idx]), copy.deepcopy(P)])  # deepcopy只复制使用，不影响原值
    ...
    def server_train(self, epoch, num_current, train_dataset, user_groups, global_model, avg_iter, idxs_users):
        ...
        if alpha > 0.001:
            ...
            if self.args.optim == "fsgdm":
                ...
                print(f"round{epoch} server fixing with iters/actual_iters/alpha {server_iter} / {actual_iter} / {alpha}")
            else:
                w, loss = local_model.update_weights(
