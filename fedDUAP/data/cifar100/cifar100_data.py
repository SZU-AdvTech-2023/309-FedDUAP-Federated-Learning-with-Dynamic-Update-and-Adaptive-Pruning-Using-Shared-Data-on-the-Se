import numpy as np
from FL_pruning_algorithm.fedDUAP.utils.util import get_root_path
from FL_pruning_algorithm.fedDUAP.data.util import show_data
...
def cifar_noniid(dataset, num_users, num_data, num_share=0, l=20, l_share=100):
    """
    ...
    for category in rand_category:
        ...
        dict_users[num_users] = np.concatenate(
            (dict_users[num_users], np.random.choice(choices, min(num_each, num_category[category]), replace=False)), axis=0)

    # show data distribution
    show_data(dataset, dict_users)
    return dict_users

def cifar_noniid_unequal(dataset, num_users, num_data, num_share=0, l=20, l_share=100):
    """
...
def get_dataset(num_data=40000, num_users=100, iid=True, unequal=False, l=20, num_share=0, share_l=100):
    ...
    train_dataset = paddle.vision.datasets.Cifar100(mode='train', transform=transform_train)

    test_dataset = datasets.Cifar100(mode='test', transform=transform_test)
