...
def show_data(train_dataset, user_groups, num_users=20):
    ...
    for i in range(num_users):
        user_cnt[i] = [0] * len(train_dataset.classes)  # class为标签或类别，len（*）表示有多少个标签或类别
        for idx in user_groups[i]:  # 遍历第i个user的样本
            _, label = train_dataset[int(idx)]  # 数据集中该样本的标签
            user_cnt[i][int(label)] += 1  # 客户i拥有的样本label对应的数量
    ...
    for k, v in user_cnt.items():
        ...
        for i in range(len(v)):
            if v[i] != 0:
                print(f"{i}:{v[i]}", end=" ")  # label：label的数量
        print(" | ", end=" ")
        idx += 1
        if idx % 5 == 0:
            print()
